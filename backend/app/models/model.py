...
class Carrera(SQLModel, table=True):
    ...
    Fec_Mod: datetime = Field(
        default_factory=datetime.utcnow
    )

    fichas: List["Fichas"] = Relationship(
...
class Fichas(SQLModel, table=True):
    ...
    asistencias: List["Asistencia"] = Relationship(
        sa_relationship_kwargs={
            "primaryjoin": "Fichas.Id_Fic == foreign(Asistencia.Id_Fic)"
        }
    )
...
class Competencia(SQLModel, table=True):
    ...
    resultados_aprendizaje: List["ResultadoAprendizaje"] = Relationship(
...
class Instructor(SQLModel, table=True):
    ...
    Tip_ide_Ins: TipoIdentificacion = Field(
        nullable=False
    )

    Num_ide_Ins: int = Field(
        nullable=False,
        unique=True
    )
    ...
    fichas_asignadas: List["FichaInstructor"] = Relationship(
        back_populates="instructor"
    )
...
class Administrador(SQLModel, table=True):
    ...
    Tip_ide_Adm: TipoIdentificacion = Field(
        nullable=False
    )

    Num_ide_Adm: int = Field(
        nullable=False,
        unique=True
    )
...
class Aprendiz(SQLModel, table=True):
    ...
    Tip_ide_Apr: TipoIdentificacion = Field(
        nullable=False
    )

    Num_ide_Apr: int = Field(
        nullable=False,
        unique=True
    )
...
class FichaInstructor(SQLModel, table=True):
    ...
    Id_Ins: int = Field(
        foreign_key="Instructor.Id_Ins",
        primary_key=True
    )

    Id_Comp: int = Field(
        foreign_key="competencia.Id_Comp",
        primary_key=True
    )
    ...
    Fec_Inicio_Comp: date = Field(
        nullable=False
    )

    Fec_Fin_Comp: date = Field(
        nullable=False
    )
    ...
    instructor: Optional[Instructor] = Relationship(
        back_populates="fichas_asignadas"
    )

    competencia: Optional[Competencia] = Relationship()
...
class ResultadoAprendizaje(SQLModel, table=True):
    ...
    ficha: Optional[Fichas] = Relationship(
        back_populates="resultados_aprendizaje"
    )
...
class Asistencia(SQLModel, table=True):
    ...
    Id_Fic: int = Field(nullable=False)
    Id_Ins: int = Field(nullable=False)
    Id_Comp: int = Field(nullable=False)
    ...
    ficha_instructor: Optional[FichaInstructor] = Relationship()
...
class Notificacion(SQLModel, table=True):
    ...
    aprendiz: Optional[Aprendiz] = Relationship(
